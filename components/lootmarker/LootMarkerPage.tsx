...
import { LootMarkerGallery } from "./gallery";
import { COPY, hrefFor, type Lang } from "../../content/lootmarker-copy";
import {
  HeroFrame,
  DetectionMask,
  PerspectiveFlatten,
...
// Loot Marker — bespoke product page (like /wren), served at lootmarker.doie.cc
// via a middleware host rewrite onto /lootmarker. Monochrome brand: the accent
// stays ink so the sign-collection metaphor reads pure black-on-paper, same
// discipline as Wren.
const brand = {
  "--color-accent": "#0a0a0a",
...
/* Placeholder monogram tile — swap for /lootmarker/lootmarker-icon.png when
   the real app-icon PNG is dropped into /public/lootmarker/. Keeps the header
   readable during setup without a 404. */
function LootMark({ size = 28 }: { size?: number }) {
  return (
    <div
      className="flex items-center justify-center rounded-[6px] border border-[var(--color-line)] bg-[var(--color-paper-soft)]"
      style={{ width: size, height: size }}
      aria-label="Loot Marker"
      role="img"
    >
      <span
        className="code-mono uppercase tracking-[0.02em] text-[var(--color-ink)]"
...
export function LootMarkerPage({ lang }: { lang: Lang }) {
  const c = COPY[lang];
      ...
      <header className="sticky top-0 z-50 border-b border-[var(--color-line)] bg-[var(--color-paper)]/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <Link href={home} className="flex items-center gap-2.5">
            <LootMark />
            <span className="text-[15px] font-medium tracking-tight">Loot Marker</span>
          </Link>
      ...
      <section className="px-6">
        <div className="mx-auto max-w-6xl border-t border-[var(--color-line)]">
          <FeatureRow
            ...
            body={c.features[0].b}
            object={<DetectionMask />}
          />
          <FeatureRow
            index="02"
            title={c.features[1].t}
            body={c.features[1].b}
            object={<PerspectiveFlatten />}
            flip
          />
          <FeatureRow
            index="03"
          ...
          <div className="mt-9 grid max-w-lg gap-px border-t border-l border-[var(--color-line)] sm:grid-cols-2">
            <div className="flex items-center justify-between border-b border-r border-[var(--color-line)] bg-[var(--color-ink)] px-5 py-4 text-[var(--color-paper)]">
              <div>
                ...
                <div className="mt-1 text-[15px] font-medium">App Store</div>
              </div>
      ...
      <footer className="px-6 py-14">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-2.5 text-center font-mono text-[12px] uppercase tracking-[0.08em] text-[var(--color-ink-soft)]">
          <span className="flex items-center gap-2.5">
            <LootMark size={20} />
            <span className="text-[var(--color-ink)]">Loot Marker</span>
